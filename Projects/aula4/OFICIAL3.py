...
def dimensoesObjto():     # Essa função pede o comprimento, a largura e a altura. E retorna o valor em (dimensões) volume:
    while True:           # Crie um laço de repetição para caso ocorra algum erro ela volta para o ínicio:
        try:              # Como a função pede números(int) caso seja digitado alguma letra o (try) e o (except) fará o programa reiniciar:
            comp = int(input('Por favor digite o comprimento do objeto (em cm)> '))
            largura = int(input('Por favor digite a largura do objeto (em cm)> '))       # Essa três variáveis coletam as informações da 1ª linha
            altura = int(input('Por favor digite a altura do objeto (em cm):>'))
...
def pesoObjto():   # Essa função pede o peso do objeto e retorna o multiplicador
    while True:    # Esta em um laço de repetição caso haja erro
        try:       # Como a função pede números(int) ou (float) caso seja digitado alguma letra o (try) e o (except) irá mostar uma mensagem dr erro e volta para o laço:
            ...
            elif 0.10 < pesObjeto <= 1.09:                                    # Pesos de 0.10 a 1.9(Kg)
                return 1.5
            elif 1.10 < pesObjeto <= 10:                                      # Pesos de 1.10 a 10(Kg)
                return 2
...
def ru4090111(borda):           # Nela cria uma borda adaptável com qualquer frase
    ...
    print('|', '!' * tam,'|')
    print('!', borda, '!')     # E no programa principal a (borda) será adptável
    print('|', '!' * tam, '|')
